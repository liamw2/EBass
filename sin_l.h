...
static uint16_t sine_lookup[NUM_SAMPLES] = {
...
float sin_l(float amplitude, float phase) {
    int sample_index = (phase / (M_PI * 2)) * NUM_SAMPLES;
    sample_index = sample_index % NUM_SAMPLES;
    uint16_t sample = sine_lookup[sample_index];
    float ret = (sample / (float)__UINT16_MAX__) * amplitude;
    return ret;
}
